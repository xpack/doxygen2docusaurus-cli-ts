...
description: Some recommendations on how to write effective documentation.
keywords:
  - author
  - guide
...
For class, struct, or union documentation, specify the header file in which they are declared.
It is displayed in the Included Headers section.
...
Define code sections using `@code`, not Markdown:
...
To render this as a bold heading only, add an empty line after `@par Example`
to terminate the paragraph sequence.

Otherwise, the following content will be indented.
...
To briefly document the members of structs, classes, or enumerations, use `/** ... */`.

Ensure the text is a properly formatted sentence, beginning with a capital letter and ending with a full stop.
...
For enumerations, the comment may also be placed before the member, although in
this case the `@brief` tag is mandatory, since the description is not shown in
