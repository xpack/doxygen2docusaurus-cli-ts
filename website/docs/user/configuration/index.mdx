...
The behaviour of the `doxygen2docusaurus` CLI tool can be customised using various configuration variables.
...
- **[listPagesAtTop](#listpagesattop)** - List pages at top level
- **[mainPageTitle](#mainpagetitle)** - Title for the main page
- **[navbarDropdownFilePath](#navbardropdownfilepath)** - Path to menu dropdown file
- **[navbarDropdownLabel](#navbardropdownlabel)** - Label for menu dropdown
...
The file path to which the menu dropdown file is written. This file must be included by the Docusaurus `docusaurus.config.js` file to generate the custom top navigation bar.
...
The file path to which the custom CSS file is written. This file must be included by the Docusaurus `docusaurus.config.js` file to apply custom styles to the API reference documentation.
