...
A typical example with all available options is:
...
```json
{
  ...
  "originalPagesNote": "For comparison, the original Doxygen html pages continue to be available via the <a href=\"pathname:///doxygen/topics.html\">.../doxygen/*.html</b></a> URLs."
}
```
...
An example of such a configuration file is from the project that renders Doxygen's own site:
...
Docusaurus does not enforce trailing slashes in URLs, but GitHub Pages redirects all URLs to include trailing slashes. Therefore, for websites hosted on GitHub, it is recommended to use trailing slashes for consistency. To ensure that the generated links include trailing slashes, configure the `trailingSlash` option in the `docusaurus-config.ts` file:
...
To include this within the Docusaurus build process, edit your `docusaurus-config.ts` file and add the following snippet to the theme configuration:
...
```ts
import doxygenApiNavbar from './docusaurus-config-navbar-doxygen.json'
```
...
const sidebars: SidebarsConfig = {
  ...
  doxygenSidebar: [doxygenSidebarItems],
}
```
