...
This package is also utilised by the VS Code extension and, until VS Code
accepts ESM dependencies, it must be
maintained as a legacy CommonJS dependency.
...
```bash
rm -rf ~/tmp/doxygen2docusaurus-test && mkdir ~/tmp/doxygen2docusaurus-test && cd ~/tmp/doxygen2docusaurus-test
npm init --yes
npm install @xpack/doxygen2docusaurus@next
```
