...
import {
  type AbstractCodeLineType,
  CodeLineDataModel,
  type AbstractHighlightType,
  HighlightDataModel,
  type AbstractListingTypeBase,
...
export class ListingTypeLinesRenderer extends ElementLinesRendererBase {
  override renderToLines(
    ...
    for (const codeline of element.codelines) {
      ...
      lines.push(
        renderCodeLinesToString(this.workspace, codeline, 'html', showAnchor)
      )
    }
...
function renderCodeLinesToString(
  ...
  const content = workspace.renderElementsArrayToString(
    element.highlights,
    type
  )
  if (content.length > 0) {
